...
delta_ch4(1)=delta_ch4_std(1);
delta_ch4_1per(1)=delta_ch4_std(1);
delta_ch4_const(1)=delta_ch4_std(1);
delta_ch4_const1940(1)=delta_ch4_std(1);

timedelta_ch4=[0];
timedelta_ch4_1per=[0];
timedelta_ch4_const=[0];
timedelta_ch4_const1940=[0];

[year,delta_ch4] = ode15s(@(year,delta_ch4) first_order(year,delta_ch4,ch4_tchange,ch4_ppbv,tau),tspan,delta_ch4(1)*ones(1,81));
delta_ch4 = diag(delta_ch4);
[year,delta_ch4_1per(index)] = ode15s(@(year,delta_ch4_1per) first_order(year,delta_ch4_1per,ch4_tchange_1per(index),ch4_ppbv(index),tau),tspan,delta_ch4_1per(index-1));
    delta_ch4_1per(index) = diag(delta_ch4_1per(index));
    [year,delta_ch4_const(index)] = ode15s(@(year,delta_ch4_const) first_order(year,delta_ch4_const,ch4_tchange_const(index),ch4_ppbv(index),tau),tspanaux,delta_ch4_const(index-1));
    delta_ch4_const(index) = diag(delta_ch4_const(index));
    [year,delta_ch4_const1940(index)] = ode15s(@(year,delta_ch4_const1940) first_order(year,delta_ch4_const1940,ch4_tchange_const(index),ch4_ppbv(index),tau),tspanaux,delta_ch4_const1940(index-1));
    delta_ch4_const1940(index) = diag(delta_ch4_const1940(index));

for index=2:length(tspan)
    delta_ch4out(index) = ode23(@(t,y) ((((ch4_tchange(index)./(1+ch4_tchange(index)))./tau).*ch4_ppbv(index))-((1./(1+ch4_tchange(index)))./tau).*y),tspanaux,delta_ch4(index-1));
    delta_ch4=[delta_ch4 delta_ch4out(index).y];
    timedelta_ch4=[timedelta_ch4 delta_ch4out(index).x];
    tspanaux=tspanaux+1;
end
